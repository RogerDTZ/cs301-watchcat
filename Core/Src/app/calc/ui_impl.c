...
double calculateExpression(const char *expression, int bin) {
    ...
    for (int i = 0; i < len; ++i) {
        ...
        if ((currentChar >= '0' && currentChar <= '9') || currentChar == '.') {
        	if (bin == 1){
        		operandStack[++operandTop] = customAtob(&expression[i]);  // 使用 atob 将字符串转换为浮点数
        	}else{
                operandStack[++operandTop] = customAtof(&expression[i]);  // 使用 atof 将字符串转换为浮点数
        	}
...
char* intToBinaryString(int num) {
    ...
    if (num == 0) {
        ...
        char* binaryStr = (char*)malloc(size + 1);  // 为字符串分配足够的空间，加上终止符 '\0'
